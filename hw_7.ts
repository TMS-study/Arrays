...
const numbers1: number[] = [5, 43, 63, 23, 90];
let numbers2: number[] = numbers1.slice(5);
...
const cats1: string[] = ['Gachito', 'Tom', 'Batman'];
...
const matrix2: string[] = matrix.join(',').split(',');
const matrix3: number[] = matrix2.map(Number);
//const matrix3 = matrix2.map((el)=> {return Number(el)});
const matrix4: number = Math.round(matrix3.reduce((prevValue, curValue) => prevValue + curValue) / matrix3.length);
